...
| Bindist tarball | ✅ Working | `ghc-9.2.8-stage1-cross-to-ppc-darwin8.tar.xz` (~210 MB compressed) on every GitHub release.  Includes `install.sh`, `cross-scripts/`, and a complete `bin/` of ppc helper binaries — `ghc`, `ghc-pkg`, `hp2ps`, `hsc2hs`, `unlit`, `ghc-iserv` (all cross-built since v0.15.0; iserv since v0.7.0, unlit since v0.14.1). |
