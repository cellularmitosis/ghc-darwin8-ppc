...
*Updated: 2026-04-24 session 11 (post-v0.6.0, PPC Mach-O runtime loader restored).*
...
2. **GHCi / TemplateHaskell partial** — the runtime Mach-O loader for PPC was restored in v0.6.0 (patch 0009).  `loadObj` + `resolveObjs` + `lookupSymbol` work for hand-compiled `.o` files; the loader engine is now alive on Tiger.  But TH end-to-end still needs the iserv plumbing on top: a PPC-built `ghc-iserv` running on Tiger, talking to host `ghc` over a pipe.  Plus stage2 native ghc work for in-process GHCi REPL.  See [docs/sessions/2026-04-24-session-11-ghci-macho-loader/findings.md](sessions/2026-04-24-session-11-ghci-macho-loader/findings.md).
