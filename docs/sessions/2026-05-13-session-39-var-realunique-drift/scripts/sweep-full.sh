...
# output (PROBE39-* lines and panic body) into logs/sweep-full/.
#
...
for n in $(seq $START $STEP $END); do
    ...
    echo "$out" > "$f"
    # Quick summary line:
    panic=$(echo "$out" | grep "^PROBE39-PANIC" | head -1 | cut -c1-200)
    addlost=$(echo "$out" | grep -c "^PROBE39-ADDLOST")
    shrink=$(echo "$out" | grep -c "^PROBE39-SHRINK")
