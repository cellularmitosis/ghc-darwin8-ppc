...
-- ghc-darwin8-ppc: a softer 'runInBoundThread' that falls back to running
-- the action in the current thread when the RTS does not support bound
-- threads (i.e. the non-threaded RTS, or — like our PPC32-darwin8 case —
-- the threaded RTS built without SMP because PPC32+gcc14 lacks 64-bit
-- atomic intrinsics in libgcc).  HsOpenSSL uses 'runInBoundThread' to
-- ensure OpenSSL sees the same OS thread for the duration of an
-- operation; on a single-OS-thread RTS that's trivially satisfied.
